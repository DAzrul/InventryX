...
class _ManagerReportPageState extends State<ManagerReportPage> with SingleTickerProviderStateMixin {
          ...
          build: (pw.Context context) {
            return [
              ...
              pw.Table.fromTextArray(
                ...
                headers: ['Product Name', 'Stock', 'Price (RM)'],
                data: productsSnap.docs
                    .where((d) => (int.tryParse(d['currentStock'].toString()) ?? 0) <= 10)
                    .map((d) => [
                  d['productName'] ?? '-',
                  d['currentStock']?.toString() ?? '0',
    ...
    try {
      ...
      for (var doc in products.docs) {
        sheet1.appendRow([
          ...
          ex.TextCellValue(doc['category'] ?? ''),
          ex.IntCellValue(int.tryParse(doc['currentStock'].toString()) ?? 0),
          ex.DoubleCellValue(double.tryParse(doc['price'].toString()) ?? 0.0),
        ]);
      }
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: BottomNavigationBar(
          currentIndex: _selectedIndex, onTap: _onItemTapped, backgroundColor: Colors.white,
          selectedItemColor: primaryBlue, unselectedItemColor: Colors.grey.shade400,
  ...
  BottomNavigationBarItem _navItem(IconData inactiveIcon, IconData activeIcon, String label) {
    return BottomNavigationBarItem(
      ...
      activeIcon: Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: primaryBlue.withOpacity(0.1), shape: BoxShape.circle), child: Icon(activeIcon, size: 22, color: primaryBlue)),
      label: label,
    );
  }
  ...
  Widget _buildReportUI(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          controller: _tabController, indicatorSize: TabBarIndicatorSize.tab, indicatorWeight: 4, indicatorColor: primaryBlue, labelColor: primaryBlue, unselectedLabelColor: Colors.grey.shade400,
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          _exportTile(Icons.picture_as_pdf_rounded, "Print / Save PDF", Colors.red, () { Navigator.pop(ctx); _generateAndPrintPDF(); }),
          const SizedBox(height: 12),
          _exportTile(Icons.table_chart_rounded, "Download Excel", Colors.green, () { Navigator.pop(ctx); _generateAndSaveExcel(); }),
          const SizedBox(height: 25),
        ]),
      ),
    );
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView(padding: const EdgeInsets.fromLTRB(20, 25, 20, 100), physics: const BouncingScrollPhysics(), children: [
          ...
          const SizedBox(height: 35), const Text("Category Distribution", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
