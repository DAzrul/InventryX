          ...
          children: [
            Container(
              ...
              child: IconButton(
                ...
                onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => NotificationPage(userRole: userRole))),
              ),
            ),
            if (hasUnread)
              Positioned(top: 12, right: 12, child: Container(width: 10, height: 10, decoration: BoxDecoration(color: Colors.red, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2)))),
          ],
        );
      },
    );
        ...
        StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            return _StatItem(
              ...
              color: Colors.orange,
              // [UPDATE] Redirect ke low stock page
...
class _StatItem extends StatelessWidget {
  ...
  const _StatItem({required this.label, required this.value, required this.icon, required this.color, required this.onTap});
...
class _TotalProductsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(28), boxShadow: [BoxShadow(color: Colors.indigo.withOpacity(0.06), blurRadius: 20, offset: const Offset(0, 10))]),
      child: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          return Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Product Breakdown", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), Text("${snapshot.data!.docs.length} Total", style: TextStyle(color: Colors.indigo.shade400, fontWeight: FontWeight.w600, fontSize: 13))]),
            const SizedBox(height: 25),
            Row(children: [Expanded(flex: 5, child: SizedBox(height: 140, child: PieChart(PieChartData(sectionsSpace: 2, centerSpaceRadius: 0, sections: sections)))), const SizedBox(width: 20), Expanded(flex: 5, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: legend))]),
          ]);
        },
      ),
    );
    ...
    return Container(
      ...
      child: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<String> labels = [];
          // [UPDATE] Gunakan Date (dd/MM) bukannya Day (Mon)
          for (int i = 6; i >= 0; i--) {
            String d = DateFormat('dd/MM').format(DateTime.now().subtract(Duration(days: i)));
          ...
          for (int i = 0; i < labels.length; i++) {
            ...
            groups.add(BarChartGroupData(x: i, barRods: [
              BarChartRodData(
                  ...
                  // [UPDATE] Warna lebih eye-friendly (Indigo/Blue Gradient)
                  gradient: const LinearGradient(colors: [Color(0xFF1E3A8A), Color(0xFF3B82F6)], begin: Alignment.bottomCenter, end: Alignment.topCenter)
              )
            ]));
          }
          ...
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Weekly Revenue", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), const Icon(Icons.analytics_outlined, color: Colors.indigo, size: 20)]),
              const SizedBox(height: 30),
              AspectRatio(
                aspectRatio: 1.7,
                child: BarChart(BarChartData(
                  // [UPDATE] Tooltip bila klik bar
                  barTouchData: BarTouchData(
                      touchTooltipData: BarTouchTooltipData(
                        getTooltipColor: (_) => Colors.indigo.shade900,
                        getTooltipItem: (group, groupIndex, rod, rodIndex) => BarTooltipItem("RM ${rod.toY.toStringAsFixed(2)}", const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 11)),
                      )
                  ),
                  maxY: maxVal * 1.3,
                  ...
                  titlesData: FlTitlesData(
                    ...
                    bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, getTitlesWidget: (v, m) => Padding(padding: const EdgeInsets.only(top: 10), child: Text(labels[v.toInt()], style: TextStyle(fontSize: 9, color: Colors.grey.shade500, fontWeight: FontWeight.w600))))),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Hot Selling", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: Colors.red.withOpacity(0.1), borderRadius: BorderRadius.circular(10)), child: const Text("Today", style: TextStyle(fontSize: 10, color: Colors.red, fontWeight: FontWeight.bold)))]),
        const SizedBox(height: 15),
        StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('sales').where('saleDate', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDay)).snapshots(),
          builder: (context, snapshot) {
            if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return const Padding(padding: EdgeInsets.symmetric(vertical: 20), child: Center(child: Text("No sales recorded today.", style: TextStyle(color: Colors.grey))));
            ...
            var sortedKeys = productSales.keys.toList()..sort((k1, k2) => productSales[k2]!.compareTo(productSales[k1]!));
            var top3 = sortedKeys.take(3).toList();
            return Column(children: top3.map((key) => _ProductListRow(name: key, count: "${productSales[key]} Sold", color: key == top3.first ? const Color(0xFFFF5B5B) : Colors.indigo.shade400)).toList());
          },
        ),
      ]),
    );
...
class _LegendRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [Container(width: 8, height: 8, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), const SizedBox(width: 10), Expanded(child: Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.black87), overflow: TextOverflow.ellipsis)), Text(count, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.grey.shade400))]));
  }
}
...
class _ProductListRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 12), child: Row(children: [CircleAvatar(radius: 18, backgroundColor: color.withOpacity(0.1), child: Icon(Icons.star_rounded, color: color, size: 18)), const SizedBox(width: 15), Expanded(child: Text(name, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14))), Text(count, style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.bold, fontSize: 12))]));
  }
}